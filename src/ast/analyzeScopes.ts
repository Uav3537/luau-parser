...
export type DeclarationNode = Identifier | TypedIdentifier | FunctionParameter
...
export function getBinding(analysis: ScopeAnalysis, id: Identifier): Binding | undefined {
...
class Analyzer {
    private nextId = 0
    private readonly bindingOf = new Map<Identifier, BindingId>()
    ...
    private declare(scope: Scope, name: string, kind: BindingKind, node: DeclarationNode): BindingId {
    ...
    private visitStatement(stmt: Statement, scope: Scope): void {
        switch (stmt.type) {
            case "LocalStatement": {
                // Initializers see the *old* bindings — `local x = x` reads
            ...
            case "FunctionDeclarationStatement": {
                // `function foo() end` / `function T.m() end` is sugar for
                // assigning to an *existing* variable — never a new
                // declaration. `.path` entries and `.method` are field /
                // method names, not variable refs.
